...
AddOption ("--destdir", action="store", dest="destdir", default = '/usr/local', help="Assumed as fake root for destination.")
...
## Install target
idir_prefix     = destdir
idir_bin        = os.path.join (destdir, 'bin')
idir_shr        = os.path.join (destdir, 'share/astroid')
idir_ui         = os.path.join (idir_shr, 'ui')
idir_app        = os.path.join (destdir, 'share/applications')
idir_icon       = os.path.join (destdir, 'share/icons/hicolor')
...
if not disable_plugins:
  inst_shr += env.Install (os.path.join (destdir, 'share/gir-1.0'), gir)
  inst_bin += env.Install (os.path.join (destdir, 'lib/girepository-1.0'), typelib)
